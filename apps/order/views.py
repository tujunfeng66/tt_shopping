...
from daily.settings import BASE_DIR
...
class OrderCommitView(View):
    ...
    def post(self, request):
        ...
        # 校验地址
        try:
            # addr = Address.objects.select_for_update()
        ...
        try:
            ...
            transaction.commit(save_id) # 数据库事务提交
            return JsonResponse({'res':7,'errmdg':'下单失败'})
...
class OrderPayView(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        alipay = AliPay(
            appid='2021000119646995',
            app_notify_url=None,  # 默认回调 url
            # 自己的私钥
            app_private_key_string=app_private_key_string,
            # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
            alipay_public_key_string=alipay_public_key_string,
            sign_type="RSA2",  # RSA 或者 RSA2 推荐RSA2
            debug=True,  # 默认 False
        )
        # 调用支付接口
